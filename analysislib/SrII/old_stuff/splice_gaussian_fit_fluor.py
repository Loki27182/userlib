...
# fluor = im_loading_raw[-1]
    maxcounts = 0
    for ii in range(int(ser['BlueMOTLoadTime']/ser['TimeBetweenExp'])-2):
        #currentCounts = np.sum(run.get_image('grating','fluorescence', 'atoms'+str(ii))[ROI[2]:ROI[3],ROI[0]:ROI[1]])
        currentCounts = np.sum(run.get_image('fluorescence', 'atoms'+str(ii))[ROI[2]:ROI[3],ROI[0]:ROI[1]])
        if currentCounts > maxcounts:
            print(currentCounts)
            maxcounts = currentCounts
            #fluor = run.get_image('grating','fluorescence', 'atoms'+str(ii))
            fluor = run.get_image('fluorescence', 'atoms'+str(ii))
...
if ser['FluorImaging']:
    ...
    fluor = run.get_image('fluorescence','atoms').astype('float') - run.get_image('fluorescence','background').astype('float')
# PrepareImage

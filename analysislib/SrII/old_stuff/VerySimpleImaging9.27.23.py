...
im_atoms = run.get_image(camera,'fluorescence', 'atoms').astype('float')
#im_probe = run.get_image(camera, 'fluorescence', 'probe').astype('float')
im_background = run.get_image(camera,'fluorescence', 'background').astype('float')
...
optical_depth = -1 * np.log(im_atoms)
...
#absorption = ax_absorption.imshow(im_absorption)
